...
# -------------------------------------------------------------- Konstanten --
CRS_DEFAULT   = 25832     # EPSG
BUFFER_DEFAULT = 20.0     # Meter

FLD_EDGE = "element_nr"
FLD_FROM = "beginnt_bei_vp"
FLD_TO   = "endet_bei_vp"
FLD_DIR  = "verkehrsrichtung"     # Werte: R / G / B


# --------------------------------------------------------- Hilfsfunktionen --
def ls_from_geom(g):
    """Sicherstellen, dass wir einen LineString bekommen."""
...
def reverse_geom(g):
    """Linie (oder Multi-Linie) umdrehen."""
    if isinstance(g, LineString):
...
def iter_coords(g):
    """Über alle Stützpunkte iterieren, egal ob LineString/Multi."""
    if isinstance(g, LineString):
        yield from g.coords
