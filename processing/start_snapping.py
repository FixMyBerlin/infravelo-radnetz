...
COLUMN_ORDER = [
    ...
    "tilda_width_source",
    # Prioritäts-Spalten für die Kandidatenauswahl (Reihenfolge entspricht der Berechnung von prio_total)
    ...
    "prio_missing_attributes", # Priorität für fehlende Attribute (-3 pro fehlendem Attribut: breite, ofm)
...
def format_candidates_priorities(candidates_with_priorities):
    ...
    # Verbinde alle Kandidaten mit Semikolon ohne Klammern
...
def set_priority_values(variant, best_osm, segment_angle, candidates_with_priorities=None):
    ...
    if best_osm is not None:
        ...
        variant["value_prio_distance_meter"] = best_osm.get('dist_to_mid', None)
        ...
        variant["prio_missing_attributes"] = best_osm.get('missing_attributes_priority', 0)  # Fehlende Attribute (-3 pro fehlendem Attribut)
...
def process_segments_batch_parallel(batch_data):
    ...
    for local_idx, seg_dict in enumerate(segments_batch):
        ...
        if not cand_idx:
            ...
            variants = create_directional_segment_variants_optimized(seg_dict, None, None)
...
def process(net_path, osm_path, out_path, crs, buffer, data_dir="./data", log_candidates=False, view=None, clip_region=None):
    ...
    # Entferne Breite-Attribut bei allen Mischverkehr-Varianten (unabhängig davon, ob Breite vorhanden ist)
    mischverkehr_mask = out_gdf['fuehr'].str.contains('Mischverkehr', case=False, na=False)
    ...
    fuehr_no_width_with_breite = (fuehr_no_width_mask & ~breite_null_mask).sum()
    fuehr_no_width_without_breite = (fuehr_no_width_mask & breite_null_mask).sum()
    if fuehr_no_width_mask.sum() > 0:
        logging.info(f"Führungsformen ohne Breitenerfordernis: {fuehr_no_width_with_breite} mit Breite (behalten), {fuehr_no_width_without_breite} ohne Breite (bleibt NULL)")
