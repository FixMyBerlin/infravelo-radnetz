...
def validate_knotenpunkt_id_uniqueness(gdf):
    ...
    logger.warning(f"⚠ WARNUNG: {len(duplicates)} Knotenpunkt-ID(s) kommen mehrfach vor!")
    logger.warning("-" * 80)
    
    for knotenpunkt_id, count in sorted(duplicates.items(), key=lambda x: x[1], reverse=True):
        logger.warning(f"  Knotenpunkt-ID '{knotenpunkt_id}' kommt {count}x vor")
...
def validate_null_knotenpunkt_ids(gdf):
    ...
    logger.warning(f"⚠ WARNUNG: {null_count} Feature(s) haben keine Knotenpunkt-ID!")
